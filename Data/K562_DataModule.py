...
class K562Module(pl.LightningDataModule):
    def __init__(self,
            batch_size   = 64,
            res          = 10000,
        ...
        self.line        = "K562"
        self.res         = res
        self.low_res_fn  = "Data/GSM1551620_HIC071_30.hic"
        self.hi_res_fn   = "Data/GSE63525_K562_combined_30.hic"
        self.step        = 50
        self.piece_size  = piece_size

    def download_raw_data(self):
        globs = glob.glob("Data/GSM1551550_HIC001_30.hic")
        found_data = (globs[0] == "Data/GSM1551620_HIC071_30.hic")
        if not found_data:
        ...
        globs = glob.glob("Data/GSE63525_K562_combined_30.hic")
        found_data = (globs[0] == "Data/GSE63525_K562_combined_30.hic")
        if not found_data:
            print("downloading from GSE ... this could take a while")
    ...
    def extract_create_numpy(self):
        ...
        globs   = glob.glob("Data/Constraints/"+self.line+"_high_chr6_res_"+str(self.res)+".txt")
        if len(globs)==0:
    ...
    def split_numpy(self):
        ...
        globs    = glob.glob("Data/Full_Mats/"+self.line+"_mat_high_chr10_res_"+str(self.res)+".npy")
        if len(globs) == 0:
    ...
    def prepare_data(self):
        ...
        globs       = glob.glob("Data/Splits/"+self.line+"_high_chr_*_res_"+str(self.res)+"_piece_"+str(self.piece_size)+str(".npy"))
        if len(globs) > 19:
            print("Ready to go")
        else:
    ...
    def setup(self, stage=None):
        if stage in list(range(4,22)):
            self.test_set  = self.K562Dataset(full=True, tvt=stage, res=self.res, piece_size=self.piece_size)
        if stage == 'fit':
            self.train_set = self.K562Dataset(full=True, tvt='train', res=self.res, piece_size=self.piece_size)
            self.val_set   = self.K562Dataset(full=True, tvt='val',   res=self.res, piece_size=self.piece_size)
        if stage == 'test':
            self.test_set  = self.K562Dataset(full=True, tvt='test',  res=self.res, piece_size=self.piece_size)
